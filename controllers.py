...
def add_vehicle(vehicle: VehicleCreate):
    ...
    cursor.close()
    connection.close()
    return {"message": "Véhicule ajouté", "vehicle_id": vehicle_id}
...
def update_vehicle(vehicle_id: int, vehicle: VehicleCreate):
    connection = get_db_connection()
    cursor = connection.cursor()
    cursor.execute("UPDATE vehicles SET model = %s, purchase_price = %s, rental_price = %s, is_sold = %s WHERE id = %s",
                   (vehicle.model, vehicle.purchase_price, vehicle.rental_price, vehicle.is_sold, vehicle_id))
    connection.commit()
    cursor.close()
    connection.close()
...
def get_vehicle_revenue(vehicle_id: int):
    ...
    cursor.execute("""
        ...
        COALESCE(SUM(
            CASE 
                WHEN o.order_type = 'purchase' THEN v.purchase_price
                WHEN o.order_type = 'rental' THEN v.rental_price * EXTRACT(DAY FROM AGE(o.return_date, o.start_date))
                ELSE 0
            END), 0) AS total_revenue
        FROM vehicles v
        LEFT JOIN orders o ON v.id = o.vehicle_id AND o.status = 'approved'
    ...
    if not revenue:
        raise HTTPException(status_code=404, detail="Véhicule non trouvé")
    return {"vehicle_id": revenue[0], "model": revenue[1], "total_revenue": revenue[2]}
